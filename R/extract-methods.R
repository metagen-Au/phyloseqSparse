...
#' @inheritParams base::Extract
#' @export
setMethod("[", c("otu_table","index","missing","missing"), function(x, i, j, ...){
  newx <- as(x, "matrix")[i, j, drop=FALSE]
  newx2<- otu_table(newx, taxa_are_rows(x) )
  return(newx2)
})
...
#' @inheritParams base::Extract
#' @export
setMethod("[", c("otu_table","missing","index","ANY"), function(x, i, j, ...){
  newx <- as(x, "matrix")[i, j, drop=FALSE]
  newx2<- otu_table(newx, taxa_are_rows(x) )
  return(newx2)
})
...
#' @inheritParams base::Extract
#' @export
setMethod("[", c("otu_table","missing","index","missing"), function(x, i, j, ...){
  newx <- as(x, "matrix")[i, j, drop=FALSE]
  newx2<- otu_table(newx, taxa_are_rows(x) )
  return(newx2)
})
